<template>
  <nav>
    <ul class="pagination">
      ...
      <li class="page-item" v-for="(page, index) in pages" :key="'li_' + index" :class="isCurrentPage(page) ? 'active' : ''">
        <a class="page-link" @click.prevent="changePage(page)">{{ page }}</a>
      </li>

      <li class="page-item" :class="pagination.current_page >= pagination.last_page ? 'disabled' : ''">
        <a class="page-link" @click.prevent="changePage(pagination.current_page + 1)">Next page</a>
      </li>
